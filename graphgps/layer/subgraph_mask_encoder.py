...
def get_subgraph_idx(batched_data):
    ...
    graph_offset = tmp[batched_data.subgraph_x_batch]
    # breakpoint()
    # print("graph_offset shape: ", graph_offset.shape)
    # print("batch_data.node2subgraph shape: ", batched_data.node2subgraph.shape)
...
def get_transpose_idx(batched_data):
    ...
    num_nodes_nod = num_nodes_per_subgraph[batched_data.subgraph_x_batch]
    # repeat for each subgraph in the graph
    ...
    graph_offset = subgraph_offset[index][batched_data.subgraph_x_batch]
...
class SUNMaskEncoder(torch.nn.Module):
    def __init__(self, num_layers, in_dim, emb_dim, num_clusters, feature_encoder, GNNConv,
                    use_transpose=False, drop_ratio=0., res=False, add_bn=True,
                    use_readout=True, use_mlp=True, subgraph_readout="sum"):
        super(SUNMaskEncoder, self).__init__()
        ...
        self.final_layers = torch.nn.Sequential(
            ...
            torch.nn.Linear(in_features=2 * emb_dim, out_features=num_clusters)
        )

    def forward(self, batched_data, cur_layer):
        x, edge_index, edge_attr = batched_data.subgraph_x, batched_data.subgraph_edge_index, batched_data.subgraph_edge_attr
        ...
        final_h_subgraph = self.final_layers(h_subgraph)
        raw_masks = F.softmax(final_h_subgraph, dim=-1)
        masks = torch.transpose(raw_masks, 0, 1)
        cur_mask = "masks_" + str(cur_layer)
        setattr(batched_data, cur_mask, masks)
        device = batched_data.subgraph_x.device
        batched_data.complement_masks = torch.ones(masks.shape).to(device) - masks
        return batched_data 
