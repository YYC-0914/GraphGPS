...
def set_cfg_posenc(cfg):
    ...
    cfg.posenc_VGNMaskEncoder = CN()
    cfg.posenc_RWSEMaskEncoder = CN()
    cfg.posenc_GIMaskEncoder = CN()

    # Common arguments to all PE types.
    for name in ['posenc_LapPE', 'posenc_SignNet',
                 'posenc_RWSE', 'posenc_HKdiagSE', 'posenc_ElstaticSE', 'posenc_VGNMaskEncoder', 'posenc_RWSEMaskEncoder', 'posenc_GIMaskEncoder']:
    ...
    for name in ['posenc_LapPE', 'posenc_SignNet', 'posenc_EquivStableLapPE', 'posenc_VGNMaskEncoder']:
        pecfg = getattr(cfg, name)
        pecfg.eigen = CN()
    ...
    for name in ['posenc_RWSE', 'posenc_HKdiagSE', 'posenc_ElstaticSE', 'posenc_RWSEMaskEncoder']:
        pecfg = getattr(cfg, name)
