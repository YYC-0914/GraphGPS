...
from graphgps.transform.transforms import (pre_transform_in_memory,
                                           ...
                                           clip_graphs_to_size)
from graphgps.loader.subgraph_transform import policy2transform
...
def preformat_ZINC(dataset_dir, name):
    ...
    Returns:
        PyG dataset object
    """
    if name not in ['subset', 'full', 'subset_subgraph']:
        raise ValueError(f"Unexpected subset choice for ZINC dataset: {name}")
    if name in ['subset', 'full']: 
        dataset = join_dataset_splits(
            [ZINC(root=dataset_dir, subset=(name == 'subset'), split=split)
            for split in ['train', 'val', 'test']]
        )
        ...
        dataset = join_dataset_splits(
            [ZINC(root=dataset_dir+name, subset=True, split=split, 
                  pre_transform=policy2transform(policy=cfg.dataset.subgraph_policy, 
                                                 num_hops=cfg.dataset.subgraph_num_hops, 
