...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet', 'RWSE', 'HKdiagSE',
                     'HKfullPE', 'ElstaticSE', 'GraphormerBias', 'VGNMaskEncoder', 'RWSEMaskEncoder', 'GIMaskEncoder']:
    ...
    # Eigen values and vectors for mask encoder
    laplacian_norm_tpye_mask = cfg.posenc_VGNMaskEncoder.eigen.laplacian_norm.lower()
    if laplacian_norm_tpye_mask == 'none':
        laplacian_norm_tpye_mask = None
    
    evals_mask, evects_mask = None, None
    if 'VGNMaskEncoder' in pe_types:
        L = to_scipy_sparse_matrix(
            *get_laplacian(undir_edge_index, normalization=laplacian_norm_tpye_mask,
                           num_nodes=N)
        )
        evals_mask, evects_mask = np.linalg.eigh(L.toarray())
        if 'VGNMaskEncoder' in pe_types:
            max_freqs=cfg.posenc_VGNMaskEncoder.eigen.max_freqs
            eigvec_norm=cfg.posenc_VGNMaskEncoder.eigen.eigvec_norm
        data.EigVals_mask, data.EigVecs_mask = get_lap_decomp_stats(
            evals=evals_mask, evects=evects_mask,
            max_freqs=max_freqs,
            eigvec_norm=eigvec_norm)
    ...
    if 'RWSEMaskEncoder' in pe_types:
        kernel_param = cfg.posenc_RWSEMaskEncoder.kernel
        if len(kernel_param.times) == 0:
            raise ValueError("List of kernel times required for RWSE")
        rw_landing = get_rw_landing_probs(ksteps=kernel_param.times,
                                          edge_index=data.edge_index,
                                          num_nodes=N)
        data.pestat_RWSE_mask = rw_landing
