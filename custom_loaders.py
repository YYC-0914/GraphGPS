...
def get_loader(dataset, sampler, batch_size, shuffle=True, follow_batch=None):
    pw = cfg.num_workers > 0
    if sampler == "full_batch" or len(dataset) > 1:
        loader_train = DataLoader(dataset, batch_size=batch_size,
                                  shuffle=shuffle, num_workers=cfg.num_workers,
                                  pin_memory=True, persistent_workers=pw, follow_batch=follow_batch)
...
def create_loader_for_subgraph():
    """
    ...
    if cfg.dataset.task == 'graph':
        ...
        loaders = [
            get_loader(dataset[id], cfg.train.sampler, cfg.train.batch_size,
                       shuffle=True, follow_batch=['subgraph_idx', 'subgraph_x'])
        ]
        ...
        if cfg.dataset.task == 'graph':
            ...
            loaders.append(
                get_loader(dataset[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False, follow_batch=['subgraph_idx', 'subgraph_x']))
